...
async function downloadTextReport() {
    ...
    const dates = Object.keys(sums[solarId]?.h24 || {});
    const numDays = dates.length || 1;

    let txt = `Energy Usage Summary: ${month}/${year}\n`;
    txt += `Cycle: 25th to 26th (Period: ${numDays} days)\n`;
    txt += `--------------------------------------------------------------------------------\n`;
    ...
    if (solarId) {
        const solTotal = getKwh(solarId, 'h24');
        txt += `Solar`.padEnd(16) + `${solTotal} units`.padEnd(18) + `(Avg: ${(solTotal / numDays).toFixed(1)} units/day)\n`;
    }
    if (breakerId) {
        const gridTotal = getKwh(breakerId, 'h24');
        const gridDay = getKwh(breakerId, 'day');
        const gridNight = getKwh(breakerId, 'night');
        txt += `Breaker`.padEnd(16) + `${gridTotal} units Total`.padEnd(18) + `(${gridDay} units solar time) `.padEnd(22) + `${gridNight} units night (Avg: ${(gridNight / numDays).toFixed(1)}/night)\n`;
    }
    ...
    EXPORT_FEEDS.forEach(f => {
        ...
        const night = getKwh(f.id, 'night');
        
        const col1 = f.name.padEnd(16);
        const col2 = (total + " units Total").padEnd(18);
        const col3 = ("(" + day + " units solar)").padEnd(22);
        const col4 = String(night).padStart(3) + " units night";
        const col5 = ` (Avg: ${(night / numDays).toFixed(1)}/night)`;
