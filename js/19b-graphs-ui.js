...
// ---- FIX: Helper to count actual night hours in the period ----
function countNightHours(startMs, endMs) {
    ...
    for (let t = startMs; t < endMs; t += step) {
        const d = new Date(t);
        const h = d.getHours();
        // Night = 5 PM (17) to 8 AM (8) – adjust if you want 5 PM–5 AM
        if (h >= 17 || h < 8) count++;
    }
    return count;
}

// ---- FIX: Corrected generateGraphReport ----
window.generateGraphReport = async function() {
    ...
    // ---- FIX: numDays for Day tab ----
    let numDays = allDays.size || 1;
    if (isDay) {
        numDays = 1;  // treat the whole 24h cycle as one day
    }

    // ---- FIX: use dynamic night hours count ----
