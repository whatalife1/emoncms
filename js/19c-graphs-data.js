...
async function _loadAndDraw() {
    ...
    // ✅ HOISTED: Safe defaults for ALL code paths including early returns
    let ghostData = null, avgData = null;
    ...
    if (graphFeedKey === 'momentflow') {
        ...
        graphDataCache = {
            ...
            ghostData: null, avgData: null
        };
        canvas.style.display = 'block';
        const reportDiv = document.getElementById('graph-report-view');
        if (reportDiv) reportDiv.style.display = 'none';
        _drawChart(canvas, bars1, bars2, nav.labels, '#facc15', '#ef4444', 'W', true, nav, bars1.length, multiData, 0, maxV, maxV, [], 0, 0, 0, '', '', '', null, null);
        stat.innerHTML = `
    ...
    try {
        ...
        if (isGridAll) {
            ...
            if (graphFeedKey === 'others') {
                ...
                _drawChart(canvas, bars, [], nav.labels, color1, null, unit, false, nav, computedLastIdx, null, 0, graphDataCache.maxV, graphDataCache.range, [], 0, 0, 0, '', '', '', null, null);
        ...
        // ─── GHOST (YESTERDAY) & MOVING AVERAGE LOGIC ─────────────────────────
        ghostData = null;
        if (graphTab === 'day' && graphDateNav === 0 && !isGridAll && !isCombined && graphFeedKey !== 'momentflow' && graphFeedKey !== 'report' && graphFeedKey !== 'others' && !isTemp) {
            const yesterdayStart = nav.startMs - 86400000;
            const yesterdayEnd = nav.endMs - 86400000;
            if (fA && fA.id) {
                const ptsGhost = await _gFetch(fA.id, yesterdayStart, yesterdayEnd, nav.interval);
                ghostData = _pointsToBars(ptsGhost, nav, graphFeedKey);
            }
        }

        avgData = null;
        if (window.graphShowAvg && bars1.length > 0) {
            const windowSize = graphTab === 'day' ? 15 : 3;
            avgData = new Array(bars1.length).fill(null);
            for (let i = 0; i < bars1.length; i++) {
                let sum = 0, count = 0;
                for (let j = Math.max(0, i - windowSize); j <= Math.min(bars1.length - 1, i + windowSize); j++) {
                    if (bars1[j] != null && bars1[j] > 0) { sum += bars1[j]; count++; }
                }
                if (count > 0) avgData[i] = sum / count;
            }
        }
        ...
        _drawChart(canvas, bars1, bars2, nav.labels, color1, color2, unit, isCombined, nav, lastIdx, multiData, minV, maxV, maxV-minV, barsTemp, graphDataCache.tempMinV, graphDataCache.tempMaxV, graphDataCache.tempRange, graphDataCache.tempUnit, graphDataCache.tempColor, graphDataCache.overlayLabel, ghostData, avgData);
...
// ─── Graph Enhancements: CSV, Snap, Avg ────────────────────────────────────
window.graphShowAvg = false;

window.downloadGraphCSV = function() {
    if (!graphDataCache) return alert('No data to export.');
    const { labels, bars1, bars2, multiData } = graphDataCache;
    let csv = 'Time,';

    if (multiData) {
        csv += multiData.map(m => m.label).join(',') + '\n';
        const n = multiData[0].data.length;
        for (let i = 0; i < n; i++) {
            csv += (labels[i] || i) + ',' + multiData.map(m => m.data[i] ?? '').join(',') + '\n';
        }
    } else {
        csv += 'Value 1' + (bars2 && bars2.length ? ',Value 2' : '') + '\n';
        const n = bars1.length;
        for (let i = 0; i < n; i++) {
            csv += (labels[i] || i) + ',' + (bars1[i] ?? '') + (bars2 && bars2.length ? ',' + (bars2[i] ?? '') : '') + '\n';
        }
    }
    ...
    a.download = `Graph_Export_${graphFeedKey}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
};

window.snapToPeak = function() {
    if (!graphDataCache) return;
    const { bars1, multiData, lastIdx } = graphDataCache;
    const data = multiData ? multiData[0].data : bars1;
    if (!data || data.length === 0) return;

    let maxVal = -Infinity, maxIdx = 0;
    const limit = lastIdx || data.length;
    ...
    const PL = 38, PR = 10, cW = rect.width - PL - PR;
    const n = data.length;

    const x = PL + (maxIdx / n) * cW;
    const centerX = PL + cW / 2;

    graphZoomLevel = Math.max(graphZoomLevel, 3.5);
    graphPanOffset = -(x - centerX) * graphZoomLevel;
...
// ─── Graph Enhancements: CSV, Snap, Avg, Ghost ─────────────────────────────
window.graphShowAvg = false;
