...
async function _loadAndDraw(forceRefresh = false) {
  ...
  try {
    ...
    if (graphFeedKey === 'battery') {
      ...
      // Smooth disconnection gaps or preserve real raw data based on toggle
      const isSmooth = window.graphBatterySmoothGaps !== false;
      if (typeof window.smoothBatterySocBars === 'function') {
        bars1 = window.smoothBatterySocBars(bars1, nav.nBars || bars1.length, isSmooth);
      }
